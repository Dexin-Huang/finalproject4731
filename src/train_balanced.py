"""
Train with balanced dataset - undersample makes to match misses.
"""
import argparse
import json
import os
...
# Key shooting joints (upper body focus - what worked before)
KEY_JOINTS = [
    0,   # nose
    5, 6,   # shoulders
    7, 8,   # elbows
...
class BalancedDataset(Dataset):
    def __init__(self, data, key_joints=KEY_JOINTS, balance=True, seed=42):
        labeled = [d for d in data if d.get('label', -1) != -1]

        makes = [d for d in labeled if d['label'] == 1]
        misses = [d for d in labeled if d['label'] == 0]

        if balance:
            # Undersample makes to match misses
            random.seed(seed)
            n_samples = min(len(makes), len(misses))
            makes = random.sample(makes, n_samples)
            misses = random.sample(misses, n_samples)
            labeled = makes + misses
        ...
        for seq in self.data:
            try:
                kp = np.array(seq['keypoints_3d'])[:, self.joint_indices, :]  # (4, n_joints, 3)
...
def main():
    ...
    parser.add_argument('--data', type=str, default='data/features/enhanced_mhr70.json')
    parser.add_argument('--output', type=str, default='models/best_balanced_model.pth')
    ...
    print("=" * 60)
    print("BALANCED TRAINING (Key Joints, Undersampled)")
    print("=" * 60)

    # Create balanced dataset
    dataset = BalancedDataset(data, balance=True)
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(np.zeros(len(labels)), labels)):
        ...
        criterion = nn.CrossEntropyLoss()  # No weights needed - balanced
    ...
    print("\n" + "=" * 60)
    print("FINAL RESULTS (Balanced Key Joints)")
    print("=" * 60)
